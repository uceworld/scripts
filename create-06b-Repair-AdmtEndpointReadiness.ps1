...
$results = foreach ($row in $targetRows) {
    ...
    if ($PlanOnly) {
        [pscustomobject]@{
            ...
            Notes = "PlanOnly: would repair LanmanServer, ADMIN$, firewall 135/445/RPC dynamic, DNS registration"
        }
        continue
    }

    try {
        $remoteResult = Invoke-TrustedRemoteCommand -ComputerName $ipAddress -Credential $localAdminCredential -LogPath $logPath -ArgumentList @($config.AdmtServerIp, $config.TargetDomainFqdn) -ScriptBlock {
            param(
                [string]$AdmtServerIp,
...
exit 0
'@ | Set-Content -Path $repairScriptPath -Encoding UTF8; Write-Host "Created $repairScriptPath" -ForegroundColor Green
